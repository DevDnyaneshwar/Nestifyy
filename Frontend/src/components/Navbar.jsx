      ...
      <div className="hidden md:flex items-center space-x-8">
        ...
        <div className="relative" ref={cityDropdownRef}>
          ...
          >
            ...
            <ChevronDown size={16} className={`ml-2 transition-transform duration-200 ${isCityDropdownOpen ? 'rotate-180' : ''}`} />
          </button>
          {isCityDropdownOpen && (
        ...
        {isAuthenticated ? (
          <>
            ...
            <NavLink to="/dashboard" className={({ isActive }) => `flex items-center gap-1 px-4 py-2 rounded-full transition-all duration-300 text-sm font-medium ${isActive ? 'bg-blue-600 text-white shadow-md' : 'text-gray-700 hover:bg-blue-50 hover:text-blue-700'}`} onClick={() => trackInteraction('click', 'nav_dashboard')}>
              <User className="w-4 h-4" />
              <span>Dashboard</span>
            </NavLink>
            {isAdmin && (
              <NavLink to="/admin-panel" className={({ isActive }) => `flex items-center gap-1 px-4 py-2 rounded-full transition-all duration-300 text-sm font-medium ${isActive ? 'bg-blue-600 text-white shadow-md' : 'text-gray-700 hover:bg-blue-50 hover:text-blue-700'}`} onClick={() => trackInteraction('click', 'nav_admin_panel')}>
          ...
          <>
            <div className="w-px h-6 bg-gray-200 mx-2"></div>
            <Link to="/login" className="px-5 py-2 rounded-full bg-blue-50 text-blue-600 transition-all duration-300 text-sm font-semibold shadow-sm hover:bg-blue-100 hover:shadow-md" onClick={() => trackInteraction('click', 'login_signup_button')}>
              Login/Signup
            </Link>
          </>
        )}
        ...
        <div ref={mobileMenuRef} className="absolute top-[72px] left-0 w-full bg-white shadow-xl py-6 px-4 border-t border-gray-200 z-40 overflow-y-auto max-h-[calc(100vh-72px)] md:hidden animate-slide-in-down">
          ...
          <div className="relative mb-6">
            ...
            >
              ...
              <ChevronDown size={18} className={`ml-1 transition-transform duration-200 ${isCityDropdownOpen ? 'rotate-180' : ''}`} />
            </button>
            {isCityDropdownOpen && (
          ...
          <div className="flex flex-col space-y-2">
            ...
            {isAuthenticated ? (
              ...
              <div className="mt-4 pt-4 border-t border-gray-200 flex flex-col space-y-3 px-4">
                <Link to="/login" onClick={() => { setIsMobileMenuOpen(false); trackInteraction('click', 'mobile_login_signup_button'); }} className="px-5 py-3 text-center text-blue-600 bg-blue-50 rounded-full transition-colors duration-200 font-semibold shadow-sm hover:bg-blue-100">
                  Login/Signup
                </Link>
              </div>
            )}
